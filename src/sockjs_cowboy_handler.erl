...
init({_Any, http}, Req, Service) ->
	...
	of
      ...
      {false, {cowboy, Req1}, _Reason} -> {ok, Req1, Service}
    end.

handle(Req, Service) ->
    {cowboy, Req3} = sockjs_handler:handle_req(Service,
					       {cowboy, Req}),
    {ok, Req3, Service}.
...
websocket_init(_TransportName, Req,
    ...
    SessionPid = sockjs_session:maybe_create(undefined,
					     Service1, Info),
    {RawWebsocket, {cowboy, Req7}} = case
				       sockjs_handler:get_action(Service, Req5)
					 of
				       {{match, WS}, Req6}
					   when WS =:= websocket orelse
						  WS =:= rawwebsocket ->
					   {WS, Req6}
				     end,
    self() ! go,

...
-define(ADV_COL(S, N),
	S#decoder{offset = N + S#decoder.offset,
...
-define(INC_LINE(S),
	S#decoder{offset = 1 + S#decoder.offset, column = 1,
...
-record(decoder,
	{object_hook = null, offset = 0, line = 1, column = 1,
	 state = null}).
...
json_encode({array, Array}, State)
    when is_list(Array) ->
    json_encode_array(Array, State);
json_encode({json, IoList}, _State) -> IoList;
...
json_encode_string(A, State) when is_atom(A) ->
    ...
    case json_string_is_safe(L) of
      true -> [?Q, L, ?Q];
      false ->
	  json_encode_string_unicode(xmerl_ucs:from_utf8(L),
				     State, [?Q])
    end;
...
make_object(V, #decoder{object_hook = N})
    when N =:= null orelse N =:= struct ->
    V;
make_object({struct, P},
...
decode_object(B, S = #decoder{state = key}, Acc) ->
    case tokenize(B, S) of
	  ...
	  decode_object(B, S3#decoder{state = comma},
			[{K, V} | Acc])
    end;
...
decode_array(B, S = #decoder{state = any}, Acc) ->
    case tokenize(B, S) of
	  ...
	  decode_array(B, S2#decoder{state = comma},
		       [Array | Acc]);
      {start_object, S1} ->
	  ...
	  decode_array(B, S2#decoder{state = comma},
		       [Array | Acc]);
      {{const, Const}, S1} ->
	  decode_array(B, S1#decoder{state = comma},
		       [Const | Acc])
    end;
...
tokenize_string_fast(B, O) ->
    case B of
      <<_:O/binary, (?Q), _/binary>> -> O;
      <<_:O/binary, $\\, _/binary>> -> {escape, O};
      <<_:O/binary, C1, _/binary>> when C1 < 128 ->
	  tokenize_string_fast(B, 1 + O);
      <<_:O/binary, C1, C2, _/binary>>
	  when C1 >= 194, C1 =< 223, C2 >= 128, C2 =< 191 ->
	  tokenize_string_fast(B, 2 + O);
      <<_:O/binary, C1, C2, C3, _/binary>>
	  when C1 >= 224, C1 =< 239, C2 >= 128, C2 =< 191,
	       C3 >= 128, C3 =< 191 ->
	  tokenize_string_fast(B, 3 + O);
      <<_:O/binary, C1, C2, C3, C4, _/binary>>
	  when C1 >= 240, C1 =< 244, C2 >= 128, C2 =< 191,
	       C3 >= 128, C3 =< 191, C4 >= 128, C4 =< 191 ->
	  tokenize_string_fast(B, 4 + O);
      _ -> throw(invalid_utf8)
    end.

tokenize_string(B, S = #decoder{offset = O}, Acc) ->
    case B of
      <<_:O/binary, (?Q), _/binary>> ->
	  {{const, iolist_to_binary(lists:reverse(Acc))},
	   ?INC_COL(S)};
      <<_:O/binary, "\\\"", _/binary>> ->
	  ...
	  if C > 55295, C < 56320 ->
		 ...
		 tokenize_string(B, ?ADV_COL(S, 12), Acc1);
	     true ->
		 R = if C < 65534 -> xmerl_ucs:to_utf8(C);
			true ->
			    [224 + (C bsr 12), 128 + (C bsr 6) band 63,
			     128 + C band 63]
		     end,
		 Acc1 = lists:reverse(R, Acc),
		 tokenize_string(B, ?ADV_COL(S, 6), Acc1)
...
tokenize_number(B, sign, S = #decoder{offset = O},
    ...
    case B of
      ...
      _ -> tokenize_number(B, int, S, [])
    end;
tokenize_number(B, int, S = #decoder{offset = O},
		Acc) ->
    case B of
...
tokenize_number(B, int1, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, C, _/binary>>
	  when C >= $0 andalso C =< $9 ->
	  tokenize_number(B, int1, ?INC_COL(S), [C | Acc]);
      _ -> tokenize_number(B, frac, S, Acc)
    end;
tokenize_number(B, frac, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, $., C, _/binary>> when C >= $0, C =< $9 ->
	  tokenize_number(B, frac1, ?ADV_COL(S, 2),
			  [C, $. | Acc]);
      <<_:O/binary, E, _/binary>>
	  when E =:= $e orelse E =:= $E ->
	  tokenize_number(B, esign, ?INC_COL(S),
			  [$e, $0, $. | Acc]);
      _ -> {{int, lists:reverse(Acc)}, S}
    end;
tokenize_number(B, frac1, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, C, _/binary>>
	  when C >= $0 andalso C =< $9 ->
      ...
      _ -> {{float, lists:reverse(Acc)}, S}
    end;
tokenize_number(B, esign, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, C, _/binary>>
	  when C =:= $- orelse C =:= $+ ->
	  tokenize_number(B, eint, ?INC_COL(S), [C | Acc]);
      _ -> tokenize_number(B, eint, S, Acc)
    end;
tokenize_number(B, eint, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, C, _/binary>>
	  when C >= $0 andalso C =< $9 ->
...
tokenize_number(B, eint1, S = #decoder{offset = O},
		Acc) ->
    case B of
      <<_:O/binary, C, _/binary>>
	  when C >= $0 andalso C =< $9 ->
...
tokenize(B, S = #decoder{offset = O}) ->
    case B of
      ...
      <<_:O/binary, "]", _/binary>> ->
	  {end_array, ?INC_COL(S)};
      <<_:O/binary, ",", _/binary>> -> {comma, ?INC_COL(S)};
      <<_:O/binary, ":", _/binary>> -> {colon, ?INC_COL(S)};
      ...
      <<_:O/binary, C, _/binary>>
	  when C >= $0 andalso C =< $9 orelse C =:= $- ->
	  tokenize_number(B, S);
      <<_:O/binary>> -> trim = S#decoder.state, {eof, S}
    end.

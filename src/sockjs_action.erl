...
% none
-export([iframe/3, info_test/3, options/3,
	 welcome_screen/3]).

% send
-export([eventsource/4, htmlfile/4, jsonp/4,
	 xhr_polling/4, xhr_streaming/4]).
...
-spec welcome_screen(req(), headers(),
		     service()) -> req().
...
info_test(Req, Headers,
	  #service{websocket = Websocket,
		   cookie_needed = CookieNeeded}) ->
    I = [{websocket, Websocket},
	 {cookie_needed, CookieNeeded}, {origins, [<<"*:*">>]},
...
-spec xhr_polling(req(), headers(), service(),
		  session()) -> req().
...
-spec xhr_streaming(req(), headers(), service(),
		    session()) -> req().

xhr_streaming(Req, Headers,
    ...
    Req2 = chunk(Req1,
		 ...
		 fun fmt_xhr/1),
    reply_loop(Req2, Session, ResponseLimit, fun fmt_xhr/1,
	       Service).

-spec eventsource(req(), headers(), service(),
		  session()) -> req().

eventsource(Req, Headers,
	    Service = #service{response_limit = ResponseLimit},
	    SessionId) ->
    Req1 = chunk_start(Req, Headers,
		       "text/event-stream; charset=UTF-8"),
    Req2 = chunk(Req1, <<$\r, $\n>>),
    reply_loop(Req2, SessionId, ResponseLimit,
	       fun fmt_eventsource/1, Service).

-spec htmlfile(req(), headers(), service(),
	       session()) -> req().

htmlfile(Req, Headers,
	 Service = #service{response_limit = ResponseLimit},
	 SessionId) ->
    S = fun (Req1, CB) ->
		...
		reply_loop(Req3, SessionId, ResponseLimit,
			   fun fmt_htmlfile/1, Service)
	end,
    verify_callback(Req, S).

-spec jsonp(req(), headers(), service(),
	    session()) -> req().
...
-spec xhr_send(req(), headers(), service(),
	       session()) -> req().
...
-spec jsonp_send(req(), headers(), service(),
		 session()) -> req().
...
handle_recv(Req, Body, Session) ->
    case Body of
	  ...
	  case sockjs_json:decode(Body) of
		...
		sockjs_session:received(Decoded, Session), ok;
	    {error, _} ->
		{error,
		 sockjs_http:reply(500, [], "Broken JSON encoding.",
				   Req)}
	  end
    end.
...
chunk_start(Req, Headers, ContentType) ->
    sockjs_http:chunk_start(200,
			    [{"Content-Type", ContentType}] ++ Headers, Req).
...
reply_loop(Req, SessionId, ResponseLimit, Fmt,
    ...
    case sockjs_session:reply(SessionId) of
	  ...
	  receive
	    ...
	    {tcp, _S, Data} ->
		error_logger:error_msg("Received unexpected data on a long-polling "
	  ...
	  reply_loop0(Req2, SessionId,
		      ResponseLimit - size(Frame2), Fmt, Service)
    end.

reply_loop0(Req, _SessionId, ResponseLimit, _Fmt,
	    _Service)
    when ResponseLimit =< 0 ->
    chunk_end(Req);
reply_loop0(Req, SessionId, ResponseLimit, Fmt,
	    Service) ->
    reply_loop(Req, SessionId, ResponseLimit, Fmt, Service).

chunk(Req, Body) ->
    {_, Req1} = sockjs_http:chunk(Body, Req), Req1.
...
chunk_end(Req, Body, Fmt) ->
    Req1 = chunk(Req, Body, Fmt), chunk_end(Req1).
...
fmt_eventsource(Body) ->
    Escaped =
	sockjs_util:url_escape(binary_to_list(iolist_to_binary(Body)),
			       "%\r\n\000"), %% $% must be first!
    [<<"data: ">>, Escaped, <<"\r\n\r\n">>].
